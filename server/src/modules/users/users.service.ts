...
export class UsersService {
  ...
  private mergeFreelancerProfile(user: Record<string, unknown>) {
    ...
    return {
      ...
      skills: typeof fp.skills === 'string' ? JSON.parse(fp.skills) : (fp.skills ?? []),
      categories: typeof fp.categories === 'string' ? JSON.parse(fp.categories) : (fp.categories ?? []),
      experience: typeof fp.experience === 'string' ? JSON.parse(fp.experience) : (fp.experience ?? []),
      education: typeof fp.education === 'string' ? JSON.parse(fp.education) : (fp.education ?? []),
      languages: typeof fp.languages === 'string' ? JSON.parse(fp.languages) : (fp.languages ?? []),
      portfolioItems: typeof fp.portfolioItems === 'string' ? JSON.parse(fp.portfolioItems) : (fp.portfolioItems ?? []),
